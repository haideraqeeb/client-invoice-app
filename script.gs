...
// ========== NEW: Parse Multiple Items ==========
/**
 * Parses descriptions and costs that may contain multiple items
 * Format: "Item1 | Item2 | Item3" and "1000 | 2000 | 3000"
 * Returns: [{description: "Item1", amount: 1000}, ...]
 */
function parseMultipleItems(descriptionStr, costStr) {
  const DELIMITER = '|';
  ...
  // Split by delimiter
  const descriptions = descriptionStr.split(DELIMITER).map(d => d.trim()).filter(d => d.length > 0);
  const costs = costStr.split(DELIMITER).map(c => c.trim()).filter(c => c.length > 0);
  
  // If no delimiter found, treat as single item
  if (descriptions.length === 1 && costs.length === 1) {
    const amount = parseFloat(costs[0].replace(/,/g, '')) || 0;
    return [{
      description: descriptions[0] || 'Service',
      amount: amount
    }];
  }
  
  // If counts don't match, try to handle gracefully
  if (descriptions.length !== costs.length) {
    console.warn('Mismatch: ' + descriptions.length + ' descriptions but ' + costs.length + ' costs');
    
    // If more descriptions than costs, use 0 for missing costs
    while (costs.length < descriptions.length) {
      costs.push('0');
    }
    
    // If more costs than descriptions, use generic description
    while (descriptions.length < costs.length) {
      descriptions.push('Item ' + (descriptions.length + 1));
    }
  }
  ...
  for (let i = 0; i < descriptions.length; i++) {
    const amount = parseFloat(costs[i].replace(/,/g, '')) || 0;
    items.push({
      description: descriptions[i] || 'Service',
      amount: amount
    });
  }
...
// ========== NEW: Generate Item Rows HTML ==========
/**
 * Generates HTML table rows for items
 * For non-GST: just list items
 * For GST: show items, then subtotal, then GST
 */
function generateItemRowsHtml(items, hasGST) {
  let html = '';
  let subtotal = 0;
  ...
  // If multiple items, add a subtotal row
  if (items.length > 1) {
    html += `<tr style="border-top: 2px solid #ccc;">
      <td style="text-align: right;"><strong>Subtotal:</strong></td>
      <td class="amount"><strong>₹ ${formatINR(subtotal)}</strong></td>
    </tr>`;
  }
...
function onSheetEdit(e) {
  ...
  try {
    ...
    const gstCol = headerMap[HEADER_NAMES.GST] || null;
    ...
    // ========== NEW: Parse multiple items ==========
    const items = parseMultipleItems(descriptionRaw, costRaw);
    ...
    let itemRowsHtml = generateItemRowsHtml(items, hasGST);
...
function convertToMultiLineAddress(address) {
  ...
  if (parts.length <= 3) return parts.join(',<br>');
  ...
  for (let i = 0; i < parts.length; i++) {
    if (i > 0 && i % partsPerLine === 0 && lineIndex < 2) lineIndex++;
